...
The multichain worker intentionally scans Base and BSC sequentially rather than starting six more concurrent workers. That keeps the shared Etherscan V2 key below the configured request rate while still covering every block range through each chain's persisted watermark. Ethereum, Base, and BSC default to `activity` discovery mode: they scan range logs for zero-address token/NFT mints, ordinary ERC20/ERC721 `Transfer` activity, ERC1155 mints, newly created DEX pool swap activity, and sparse claim/mine/mint/reward custom event topics, aggregate interaction counts, and only check newly created contracts after they cross the chain's activity threshold. The default threshold is 100 observations on every chain. This avoids per-block `eth_getBlockByNumber` deployment scans and ignores tiny projects until they show real propagation. The default Ethereum profile scans up to 300 blocks per 30-minute cycle. Base assumes about 2s blocks, scans 300-block batches, and can run up to 10 catch-up batches per 30-minute cycle, enough to cover normal Base production while avoiding one oversized log query window. BSC follows the post-Fermi sub-second cadence with about 0.45s blocks and scans up to 5000 blocks per 30-minute cycle. If downtime leaves a larger lag, `run-multichain-once.ps1` runs up to `-MaxCatchupBatches 10` scan batches per chain before sleeping, so it can backfill more than one normal interval without needing real-time polling.

Use `start-background-all.ps1` after a machine reboot or interruption when you want the standard three-worker topology restored in the correct order. It runs stale processing recovery once before any worker starts, then starts the scanner, the Etherscan-only classifier, and the OpenTwitter enricher, preserving the scanner's default scan-only mode and the enricher's stale-only recovery default. The stop scripts clean empty, invalid, and dead pid files, and their orphan-process fallback distinguishes scan-only and enrich-only workers even when `--max-blocks` was passed as `--max-blocks=0` or with quotes. Use the individual scripts when intentionally changing one worker's parameters.
